...
class ADHA:
    def __init__(self, angle_file, anglesum_file, epochs=600):
        ...
        anglesum = pd.read_excel(anglesum_file, usecols=['ADHA_res'])

        # Convertir los datos a arrays de numpy
        self.ids = angle['Angulo'].to_numpy(dtype=float)
        self.suma_ids = anglesum['ADHA_res'].to_numpy(dtype=float)
    ...
    def predecir(self, valor):
        ...
        if ajustador <= 5868 or ajustador >= 6000:
...
predictor = ADHA('DataBase.xlsx', 'DataBase.xlsx')

# Crear la aplicación Flask
app = Flask(__name__)

@app.route('/predecirADHA', methods=['POST'])
def predecirADHA():
    data = request.json
