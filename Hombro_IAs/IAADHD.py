...
class ADHD:
    def __init__(self, angle_file, anglesum_file, epochs=600):
        ...
        anglesum = pd.read_excel(anglesum_file, usecols=['ADHD_res'])

        # Convertir los datos a arrays de numpy
        self.ids = angle['Distancia'].to_numpy(dtype=float)
        self.suma_ids = anglesum['ADHD_res'].to_numpy(dtype=float)
    ...
    def predecir(self, valor):
        ...
        if ajustador <= 663 or ajustador >= 714:
...
predictor = ADHD('DataBase.xlsx', 'DataBase.xlsx')

# Crear la aplicación Flask
app = Flask(__name__)

@app.route('/predecirADHD', methods=['POST'])
def predecirADHD():
    data = request.json
