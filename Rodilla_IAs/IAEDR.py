...
class EDR:
    def __init__(self, angle_file, anglesum_file, epochs=600):
        ...
        anglesum = pd.read_excel(anglesum_file, usecols=['EDR_res'])

        # Convertir los datos a arrays de numpy
        self.ids = angle['Angulo'].to_numpy(dtype=float)
        self.suma_ids = anglesum['EDR_res'].to_numpy(dtype=float)
    ...
    def predecir(self, valor):
        ...
        if ajustador <= 3262 or ajustador >= 3430:
...
@app.route('/predecirEDR', methods=['POST'])
def predecirEDR():
    data = request.json
