...
class FRD:
    def __init__(self, angle_file, anglesum_file, epochs=600):
        ...
        anglesum = pd.read_excel(anglesum_file, usecols=['FRD_res'])

        # Convertir los datos a arrays de numpy
        self.ids = angle['Angulo'].to_numpy(dtype=float)
        self.suma_ids = anglesum['FRD_res'].to_numpy(dtype=float)
    ...
    def predecir(self, valor):
        ...
        if ajustador <= 894 or ajustador >= 918:
...
@app.route('/predecirFRD', methods=['POST'])
def predecirFRD():
    data = request.json
