...
class IAFlexionRodilla:
    def __init__(self, archivo_angulos, archivo_resultados, epocas=600):
        ...
        self.resultados = pd.read_excel(archivo_resultados, usecols=['FDR_res']).to_numpy(dtype=float).flatten()
...
@app.route('/recomendacion_flexion_rodilla', methods=['POST'])
def recomendacion_flexion_rodilla():
    data = request.json
